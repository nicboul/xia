...
{
	journal_strace("acl_get_context");
...
{
	journal_strace("acl_swap_context");
	return swap_context(name, acl_get_context(name));
}

uint8_t __acl_exec(struct acl_msg *aclm, uint8_t category)
{
	journal_strace("__acl_exec");
...
struct muxia_info *acl_exec(void *msgp)
{
	journal_strace("acl_exec");
	...
	switch (node_type) {
		case XIA_NODE_TYPE_USR:
			mux.ret = __acl_exec(aclm, ACL_CATEGORY_USR);
			break;
		...
		case XIA_NODE_TYPE_RES:
			mux.ret = __acl_exec(aclm, ACL_CATEGORY_RES);
			break;

		case XIA_NODE_TYPE_BRIDGE_RES:
		case XIA_NODE_TYPE_TUNNEL:
			mux.ret = __acl_exec(aclm, ACL_CATEGORY_ROOT);
			break;
...
int acl_init()
{
	...
	mem_fun.set_init();
	muxia_register(&acl_exec, XIAMSG_ACL);
