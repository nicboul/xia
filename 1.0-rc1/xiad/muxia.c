...
{
	journal_strace("muxia_fini");
	free(MBUF);
}

int muxia_init(char *in_addr, int port)
{
	journal_strace("muxia_init");
	...
	ret = netbus_newserv(in_addr, port,  NULL, NULL, muxia);
	if (ret < 0) {
