...
int discovery_send(struct discovery_msg *msg)
{
	...
	msg->version = XIA_MSG_DISCOVERY_VERSION;
...
int discovery_set_res_addr(reslist_t *entry)
{
	...
	msg.type = XIA_MSG_DISCOVERY_SET_RES_ADDR;
	msg.addr.s_addr = entry->addr.s_addr;
	memcpy(msg.haddr, entry->chaddr, XIA_MSG_DISCOVERY_HADDRSIZ);
...
int discovery_set_res_hostname(reslist_t *entry)
{
	...
	msg.type = XIA_MSG_DISCOVERY_SET_RES_HOSTNAME;
	memcpy(msg.haddr, entry->chaddr, XIA_MSG_DISCOVERY_HADDRSIZ);
	strncpy((char *)&msg.hostname, entry->hostname,
	    XIA_MSG_DISCOVERY_HOSTNAMESIZ);
...
int discovery_set_res_alive(reslist_t *entry)
{
	...
	msg.type = XIA_MSG_DISCOVERY_SET_RES_HOST_ALIVE;
	memcpy(msg.haddr, entry->chaddr, XIA_MSG_DISCOVERY_HADDRSIZ);
...
int discovery_set_res_dead(reslist_t *entry)
{
	...
	msg.type = XIA_MSG_DISCOVERY_SET_RES_HOST_DEAD;
	memcpy(msg.haddr, entry->chaddr, XIA_MSG_DISCOVERY_HADDRSIZ);
