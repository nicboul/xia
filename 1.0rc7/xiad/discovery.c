...
void discovery_msg_print(struct discovery_msg *msg)
{
	...
	journal_notice("discovery msg haddr   : %02x:%02x:%02x:%02x:%02x:%02x\n",
	    msg->haddr[0], msg->haddr[1], msg->haddr[2], msg->haddr[3],
	    msg->haddr[4], msg->haddr[5]);
...
struct muxia_info *discovery_demux(void *msgp)
{
	...
	char *addr, haddr[2 * DISCOVERY_MSG_HADDRSIZ + 1];
	...
	snprintf(haddr, sizeof(haddr), "%02x%02x%02x%02x%02x%02x",
	    msg->haddr[0], msg->haddr[1], msg->haddr[2], msg->haddr[3],
	    msg->haddr[4], msg->haddr[5]);
	...
	switch (msg->type) {
		...
		case DISCOVERY_MSG_SET_RES_ADDR:
			dbal_set_res_addr_mapping(msg->name, haddr,
			    inet_ntoa(msg->addr));
			break;

		case DISCOVERY_MSG_SET_RES_HOSTNAME:
			if (strlen(msg->hostname) == 0)
				snprintf(msg->hostname, sizeof(msg->hostname),
				    DISCOVERY_SET_RES_HOSTNAME_DEFAULT);
			dbal_set_res_hostname_mapping(msg->name, haddr, msg->hostname);
			break;

		case DISCOVERY_MSG_SET_RES_HOST_DEAD:
			dbal_set_res_status(msg->name, haddr, 0);
			break;

		case DISCOVERY_MSG_SET_RES_HOST_ALIVE:
			dbal_set_res_status(msg->name, haddr, 1);
			break;
	}
